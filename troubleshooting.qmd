...
#| echo: false
#| class-output: bash
cat(paste0("squeue -A ",id_project," | sort -k 4"))
```
...
#| echo: false
#| class-output: bash
cat(paste0("squeue -A ",id_project," | tr -s ' ' | cut -d' ' -f5 | sort | uniq -c | sort -k1"))
```
...
#| echo: false
#| class-output: bash
cat(paste0("squeue -A ",id_project," -o %C | awk '{total += $0} END{print total}'"))
```
...
#| echo: false
#| class-output: bash
cat(paste0("uquota | grep ",id_project))
```
...
#| echo: false
#| class-output: bash
cat(paste0("bash /sw/courses/utils/list_modification_times.sh /proj/",id_project,"/nobackup/"))
```
...
#| echo: false
#| class-output: bash
cat(paste0("scontrol show res | grep ",id_project))
```
...
#| echo: false
#| class-output: bash
cat(paste0("projmembers ",id_project))
```
