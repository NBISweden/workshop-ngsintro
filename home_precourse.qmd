...
![](assets/images/precourse/xquartz.webp){.precourse-tool} X11 is required to open graphical windows from a terminal. [{{< fa brands apple >}}]{.badge} Mac users will need to download and install [XQuartz](https://www.xquartz.org/) if not already installed. You can check if it's installed by running `which Xquartz` in the terminal.  
[{{< fa brands windows >}}]{.badge} Windows users can install [Xming](https://sourceforge.net/projects/xming/) to open windows from PowerShell. If you are using MobaXterm, it already has X11 support built in.
...
[![](assets/images/precourse/mobaxterm.webp){.precourse-tool}](http://mobaxterm.mobatek.net) [{{< fa brands windows >}}]{.badge} If you are on a Windows system, [MobaXterm](http://mobaxterm.mobatek.net) is a great all-in-one terminal solution. It has a terminal, opens X11 windows and allows browsing remote files graphically through SFTP. It is recommended that you INSTALL the program and not use the portable version.
