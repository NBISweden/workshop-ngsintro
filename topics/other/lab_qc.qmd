...
path_fastq: "{{< meta path_resources >}}/vc/data/fastq"
format: html
---
...
```sh
ln -s {{< meta path_fastq >}}/HG00097_1.fq
ln -s {{< meta path_fastq >}}/HG00097_2.fq
ln -s {{< meta path_fastq >}}/HG00100_1.fq
ln -s {{< meta path_fastq >}}/HG00100_2.fq
ln -s {{< meta path_fastq >}}/HG00101_1.fq
ln -s {{< meta path_fastq >}}/HG00101_2.fq
```

### Inspecting the raw reads

The files come from paired-end sequencing, where each fragment is sequenced from two ends.
The two reads are sometimes called R1 and R2, and are typically stored in two different files with file names
that have a `1` and a `2` in their names.

Use the program `less` to look at the contents of the FASTQ file containing the R2 reads of the `HG00101` sample:

```sh
less HG00101_2.fq
```

While `less` is running, use the arrow keys to scroll up and down. Use the `Q` key to quit the program when you are done.
