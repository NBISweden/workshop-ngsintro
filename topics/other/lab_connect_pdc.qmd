...
1. Start ThinLinc.  
  ![](assets/thinlinc_pdc_000.png)
1. Press `Options`, select the `Security` tab and select authentication method `Public key`.
  ![](assets/thinlinc_pdc_001.png)
1. Enter server `dardel-vnc.pdc.kth.se`, your PDC username, and the path to your private SSH key.  
  ![](assets/thinlinc_pdc_002.png)
1. Press `Connect`, and enter the password to unlock your SSH key when asked.
1. When you are done with working with the remote desktop, press the &#9211; power icon in the top-right corner of the screen and select `Log Out`.
