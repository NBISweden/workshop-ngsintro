...
The reason we show you different ways is that some parts of this course will require you to view plots and images, and adding an additional download step would just unnecessarily complicate things.
...
Add whatever is prewritten in the field `define one yourself`, usually something like `*.your.internetprovier.com`. This will allow you to use this key as long as you are connecting from the same location as you are connecting now. If you want to use this key from another in the future you can add a similar pattern that matches e.g. your home university's (or internet provider) hostname (e.g. `*.uu.se`, `*.kth.se` or `*.telia.com`).
...
For more information, take a look at PDC's website: <a href="https://support.pdc.kth.se/doc/login/interactive_hpc/" target="_blank" rel="noopener noreferrer">https://support.pdc.kth.se/doc/login/interactive_hpc/</a>
