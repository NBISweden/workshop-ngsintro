...
```{r,include=FALSE}
## VARIABLES
path_reseq <- file.path(path_resources,"reseq")

datadir <- file.path(path_reseq, "data")
fastqdir <- file.path(path_reseq, "data/fastq")
refdir <- file.path(path_reseq, "data/ref")
bamdir <- file.path(path_reseq, "data/bam")
vcfdir <- file.path(path_reseq, "data/vcf")
bpbamdir <- file.path(path_reseq, "data/best_practise_bam")
bpvcfdir <- file.path(path_reseq, "data/best_practise_vcf")
col_uppmax <- "#f4f8e8"
col_local <- "#e5f4f8"
```
...
This lab will be done completely on Dardel and the instructions assume that you connect via ThinLinc.
However, if you prefer to connect to Dardel via ssh you can instead copy some of the resulting files to your laptop and work on them there. , install IGV, and run all the IGV steps on your laptop.
If so, please create a local workspace on your laptop, for example a folder called *vc* on your desktop. You need to have write permission in this folder.
If you connect to Dardel via ThinLinc you don't have to create a local workspace.

## Dardel {-}
### Connect to Dardel {-}

During this lab it is best to connect to Dardel with ThinLinc, which gives you a graphical remote desktop. Instructions for this is available at [**Connecting to Dardel**](topics/other/lab_connect_pdc.html). Please follow the instructions in section 1.2 Remote desktop connection.
...
Use this code to allocate a job:

```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('salloc -A ',id_project,' -t 04:00:00 -p shared -c 4 --no-shell'))
```
...
```bash
gatk --java-options -Xmx5g CreateSequenceDictionary -R human_g1k_v37_chr2.fasta -O human_g1k_v37_chr2.dict
```
...
To use IGV on Dardel we recommend that you are connected via ThinLinc.
Alternatively you can install IGV on your local computer, download the files using scp, and look at them locally.
The instructions below assume that you have logged in to Dardel via ThinLinc.\
...
In the `File` menu, select `Load from File`, navigate to your workspace and for the lab, and select `HG00097.bam`. This data in this file should then appear in the tracks window.\
You need to Zoom in to see the reads. You can either select a region by click and drag, or by typing a region or a gene name in the text box at the top. Remember that we have data for the region chr2:136545000-136617000.\
...
-t 14 human_g1k_v37_chr2.fasta sample_1.fq sample_2.fq | samtools sort > sample.bam
```
...
cat(paste0('gatk --java-options -Xmx4g BaseRecalibrator \\
    -R human_g1k_v37_chr2.fasta \\
    -I HG00097.md.bam \\
...
To learn more about SLURM and SBATCH scripts please look the [How to Run Jobs](https://www.pdc.kth.se/support/documents/run_jobs/job_scheduling.html) page on the PDC website.
...
do
  ...
  -t 14 human_g1k_v37_chr2.fasta "${sample}_1.fq" "${sample}_2.fq" | samtools sort > "${sample}.bam"
