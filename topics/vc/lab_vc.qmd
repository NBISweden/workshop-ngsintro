...
path_vc: "{{< meta path_resources >}}/vc"
path_data: "{{< meta path_resources >}}/vc/data"
path_fastq: "{{< meta path_resources >}}/vc/data/fastq"
path_ref: "{{< meta path_resources >}}/vc/data/ref"
path_bam: "{{< meta path_resources >}}/vc/data/bam"
path_vcf: "{{< meta path_resources >}}/vc/data/vcf"
path_bpbam: "{{< meta path_resources >}}/vc/data/best_practise_bam"
path_bpvcf: "{{< meta path_resources >}}/vc/data/best_practise_vcf"
format: html
editor:
  markdown: 
    wrap: sentence
...
You should work in a subfolder of the course folder on the HPC, like you have done during the previous labs.
...
```sh
ln -s {{< meta path_ref >}}/human_g1k_v37_chr2.fasta
...
Use this code to allocate a job (note that the reservation changes each day). It is important that you allocate a new job with -c 8 to get enough resources for some of the steps.
...
Replace *nodename* with that name.
...
Remember that these modules must be loaded **every time you login to the HPC,** or when you connect to a new compute node.
Load the PDC module first to make it possible to load the other modules:
...
Navigate to your local workspace (do not log in to Dardel).
...
In the `File` menu, select `Load from File`, navigate to your workspace, and select `HG00097.bam`. This data should now appear as a track in the tracks window.
...
5.  Hover the mouse over the lower row of the vcf track and look under "Genotype Information". What genotype does HG00097 have at position chr2:136545844? Is this the same as you found by looking directly in the vcf file in question 1B:5?\
...
# Part 2: Variant calling in cohort {#jointvc}
...
Also check that the sample name (SM) is correct in the bam header (replace sample with the sample id):
...
You can check that your file contains 3 samples using e.g. "grep CHROM cohort.vcf"
...
do
  ...
  -t 8 human_g1k_v37_chr2.fasta \
  "${sample}_1.fq" \
  "${sample}_2.fq" | samtools sort > "${sample}.bam"
...
```sh
ln -s {{< meta path_ref >}}/1000G_phase1.snps.high_confidence.b37.chr2.vcf
ln -s {{< meta path_ref >}}/1000G_phase1.snps.high_confidence.b37.chr2.vcf.idx
```
...
Next, `HaplotypeCaller` should be run for all three samples, and the output should be in g.vcf, as described [above](#generategvcf). This time use the recalibrated bam files as input.
...
```
{{< meta path_bam >}}/HG00097.bam
{{< meta path_bam >}}/HG00100.bam
{{< meta path_bam >}}/HG00101.bam
```

If you run out of time you can click below to get paths to the precomputed cohort.g.vcf and cohort.vcf files.

```
{{< meta path_vcf >}}/cohort.g.vcf
{{< meta path_vcf >}}/cohort.vcf
```
...
- Path to intermediary and final bam files: `{{< meta path_bpbam >}}`
- Path to intermediary and final vcf files:  `{{< meta path_bpvcf>}}`
