...
During this lab it is best to connect to Dardel with ThinLinc, which gives you a graphical remote desktop. Instructions for this is available at [**Connecting to PDC**](../other/lab_connect_pdc.html). Please follow the instructions in section 1.2 Remote desktop connection.
...
```bash
salloc -A {{< meta id_project >}} --reservation={{< meta id_slurm_res_wed >}} -t 04:00:00 -p shared -c 4 --no-shell
```

Use this code to allocate a job on Thursday:

```bash
salloc -A {{< meta id_project >}} --reservation={{< meta id_slurm_res_thu >}} -t 04:00:00 -p shared -c 4 --no-shell
```
...
IGV uses java, and unfortunately it uses another version of Java than the installed version of GATK. Therefore you will not be able to use IGV and GATK in the same terminal window (this might be different if you work on another compute system later on!). 
Because of this you need to open a new terminal window on Dardel to run IGV. 
Open a new terminal window and navigate to your workspace for this lab. Use pwd to confirm that you are standing in the correct directory:
...
cat(paste0('gatk --java-options -Xmx2g BaseRecalibrator \\
    -R human_g1k_v37_chr2.fasta \\
    -I HG00097.md.bam \\
