...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('salloc -A ',id_project,' -t 04:00:00 -p shared --mem=6Gb --no-shell'))
```
...
You should work in your folder under the course’s project folder, just like you have done during the previous labs.

Start by going there using this command:

```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('cd ', path_workspace, '/vc'))
```
...
-t 14 human_g1k_v37_chr2.fasta HG00097_1.fq HG00097_2.fq | samtools sort > HG00097.bam
```
