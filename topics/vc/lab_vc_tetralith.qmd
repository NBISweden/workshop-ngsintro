...
library(yaml)
library(here)
id_project <- yaml::read_yaml(here("_quarto.yml"))$id_project_backup
...
datadir <- file.path(path_reseq, "reseq/data")
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('tetralith1$ interactive -A ',id_project,' -t 04:00:00 -n 3'))
```
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('n424$ singularity shell -B ',path_workspace,':',path_workspace,' ',path_workspace,'/ngsintro.sif'))
```
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('offline-uppmax$ mkdir ',path_workspace,'/vc\n'))
cat(paste0('offline-uppmax$ cd ',path_workspace,'/vc'))
```
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('tetralith$ ',path_workspace,'/vc\n'))
```
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('scp <nsc_username>@tetralith.nsc.uu.se:',path_workspace,'/vc/HG00097.bam* .'))
```
...
cat(paste0('scp <nsc_username>@tetralith.nsc.uu.se:',path_workspace,'/vc/HG00097.vcf* .'))
```
