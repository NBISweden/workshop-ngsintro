...
```sh
wget {{< meta custom.website.site-url >}}{{< meta output-dir >}}/topics/linux/assets/linux_advanced.tar.gz
...
The `$()` tells bash to run something in a subshell, which pretty much means it will run the command within the parentheses and then take whatever that command printed to the screen and replace the parentheses expression:
