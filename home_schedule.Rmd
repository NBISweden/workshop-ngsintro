...
s <- s %>%
  ...
  # style topic
  dplyr::mutate(topic=ifelse(tolower(topic)=="coffee break" | tolower(topic)=="lunch" | tolower(topic)=="dinner",paste0("<span style='color:white;font-weight:bold;border-radius:4px;padding:4px;background:#BDBDBD;'>",topic,"</span>"),paste0("<span style='margin-right:5px;'>",topic,"</span>"))) %>%
  # add links to topic
  ...
  mutate(topic=ifelse(is.na(link_lab),topic,paste0("<span>",topic,"<a href='",link_lab,"'><i class='fas fa-file-alt' style='margin-left:4px;margin-right:4px;'></i></a></span>"))) %>%
   mutate(topic=ifelse(is.na(link_youtube),topic,paste0("<span>",topic,"<a href='",link_youtube,"'><i class='fab fa-youtube' style='margin-left:4px;margin-right:4px;'></i></a></span>"))) %>%
...
<!-- change notice. comment this out when finalised -->
<div class="boxy boxy-lightbulb">
This schedule is not finalised and may be updated.
</div>

<!-- Coffee breaks are planned for 10:15 and 15:00 every day. -->
