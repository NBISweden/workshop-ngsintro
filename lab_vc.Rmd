...
* A common mistake is to attempt to use input files that do not exist, or create output files where you don't have permission to write.
...
```
Check to see that several new files have been created using `ls -l`. Then Generate a samtools index:
```bash
...
2. What does "SN:2" and "LN:243199373" in the "@SQ tag of the bam header mean?
...
Now we will detect short variants in the bam file using GATK's `HaplotypeCaller`. This should be done in your *UPPMAX workspace*. First use pwd to check if you are standing in the correct directory:
```bash
pwd
```
Should return
...
```{r,accordion=TRUE,chunk.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('gatk --java-options -Xmx7g MarkDuplicates \
    -I HG00097.bam \
    -O HG00097.md.bam \
...
cat(paste0('gatk --java-options -Xmx7g ApplyBQSR \
    -R human_g1k_v37_chr2.fasta \
    -I HG00097.md.bam
...
```{r,accordion=TRUE,chunk.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('gatk --java-options -Xmx7g MergeVcfs \
...
do
  echo "Now analyzing: "$sample
  bwa mem -R "@RG\tID:readgroupx\tPU:flowcellx_lanex\tSM:"$sample"\tLB:libraryx\tPL:illumina" \
