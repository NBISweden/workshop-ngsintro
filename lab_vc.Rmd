...
**NOTE: the read group is specific for each sample, so if you perform this step for another sample later on you need to change the sample tag from HG00097 to the new sample name.** 
...
## BWA mem 
Run `BWA mem` for all three samples in the data set. `BWA mem` should be run exactly [as above](#bwamem) but with the new sample names. Please note that you also need to adjust the sample tag in the read group because the sample tag needs to be unique for each sample in the cohort. 
