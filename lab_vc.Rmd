...
In the call to `BWA mem` you need to add something called a *read group*, which contain information about how the reads were generated. Read group information is required by HaplotypeCaller. Since we don't know exactly how the reads in the 1000 Genomes were generated we will have to make some assumptions and generate a "dummy" read group. Let's assume that each fastq file was generated from one library preparation (*libraryx*), derived from one biological sample (*HG00097*), that was run on one lane of a flowcell (*lanex_flowcellx*) on the Illumina machine, and let's define a readgroup (*readgroupx*) with this information. The code for adding this readgroup is:  
`-R "@RG\\tID:readgroupx\\tPU:lanex_flowcellx\\tSM:HG00097\\tLB:libraryx\\tPL:illumina"`. 

You don't need to understand the details about read groups here, but if ou would like to know more please read this article at [GATK-forum](https://gatk.broadinstitute.org/hc/en-us/articles/360035890671-Read-groups). The most important thing to know for today's workshop is that **readgroups are needed for HaplotypeCaller**, and **the SM flag should be the same as the sample name**, so you have to modify the code when running BWA for another sample. 
...
You need to specify how many threads `BWA mem`should use, and this should be the same as the number of cores you have access to. This is defined by the code `-t 1`below.

Finally, you have to use a file redirect > for the output, otherwise it will be written to stdout (your screen).
