...
You should run the lab on a compute node (not the login node). If you haven't already booked a node today, please start with that. Before you book a new node, please check that you don't already have any active jobs using this command:
```bash
squeue -u username
```
If no jobs are listed for your username please go ahead and reserve a node. If you already have an active reservation please connect to that node using ssh as described below.   
Use this code to make a reservation on day 1 of variant-calling:
