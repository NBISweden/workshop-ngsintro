...
2. If you have time, the next part of the workshop will show you how to perform joint variant calling in three samples. The aim here is that after this section you should be able to explain the differences between the g.vcf and vcf file formats. Also, if you are intetersed in learnding programming, the aim is to be able to combine individual linux commands into an SBATCH script.  
...
```
It is recommended that you specify the version number for reproducibility if you want to rerun the  exact same analyses later. Some programs, for example GATK, has different command syntax in different versions and therefore is important to know which version you used. The GATK commands in this tutorial is for GATK4.  
...
## Index the genome
Tools that compare short reads with a large reference genome needs an index of the reference genome to allow efficient random access to it. Before you can begin analysing the samples you therefore need to create index files for each tool. The generated files are like "tables of contents" that the software use to reach specific parst of the reference genomes. You don't have to understand the details of the index files to continue with this lab.
...
Now let's start the main part of the workshop, which will guide you through a basic variant calling workflow in one sample.  
  
## Quality control
![](data/ngs-workflow/main_qc.png)
  
### FastQC  
It is always recommended to perform quality control of new data, for example using FastQC, before alignment and variant calling. We analyzed these samples with FastQC earlier in the week. Please feel free to go back to the QC session if you want to repeat this, otherwise you can continue with alignment.  
...
17. What does it mean if a variant has two colours in the upper row?  
18. What colour do heterozygote variants have in the second row?  
19. What does it mean if a variant is coloured in turquoise in the second row?  
...
You can run all commands one by one in the terminal as you did in the first step of the workshop. We have also made all intermediary files avaiable in case you don't have time to complete all steps, please see links under each analysis step below. 
  
*Optionally - If you are interested and have time*, this is also good opportunity to learn how to write a simple SBATCH script and run all steps automatically. Please chech out the [SBATCH script](#sbatchscript) section below if you would like to try this. 
...
If you run out of time, please click on the arrow to get a link precomputed bam files: 
...
If you run out of time you can use the precompiled g.vcf files available in
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0(vcfdir))
```
...
```
If you didn't have time to make the cohort.g.vcf file it is available in  
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0(vcfdir))
```
Please use this file to answer the questions below.
...
31. Did the extra refinement steps in GATK's best practices short variant discovery make any difference to these variants? 
  
**Note:** In this workshop we use public data from low coverage sequencing, that has been processed before publication. The difference between variants processed with our simplified workflow and the best practices workflow may be small in this case, but with a real-life data set it is recommended to follow the best practices workflow!
