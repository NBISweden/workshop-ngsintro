...
You also need to adjust the read group information so that it matches each new sample name, like this:
```
-R "@RG\\tID:<readgroup_<sample>\\tPU:lanex_flowcellx\\tSM:<sample>\\tLB:libraryx\\tPL:illumina"
```
Where `<sample>` should be replaced with the real samples name, i.e. HG00097, HG00100 and HG00101.

Please use this command for every sample to align the reads, add the read group, sort the reads and write them to a bam file:
...
The first step in GATK's best pracice variant calling workflow is to run BWA mem exactly as you did for the three samples above. Please use the bamfiles generated with BWA mem as described in the [joint variant calling](#bwa_joint) as input for the subsequent steps in this part of the workshop.
