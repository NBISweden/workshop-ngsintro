...
In this workshop we will detect genetic variants in the region chr2:136545000-136617000 in the three samples listed above, which will allow us to check if they carry the allele for lactase persistence.
...
This lab will be done completely on Uppmax and we assume that you have logged in to Uppmax using `ThinLink`.
However, if you prefer to log in via normal `ssh` you can instead copy some of the resulting files to your laptop, install `IGV`, and run all the `IGV` steps on your laptop.
If so, please start by creating a local workspace on your laptop, for example a folder called *ngsworkflow* on your desktop. You need to have write permission in this folder.
If you work via `ThinLink` you don't have to crete a local workspace.

## UPPMAX {-}

Please connect to the Rackham cluster on UPPMAX using `thinlink` and open a terminal window as described in the "Connecting to Uppmax" lab.
...
```bash
bwa mem -R "@RG\tID:readgroup_HG00097\tPU:lanex_flowcellx\tSM:HG00097\tLB:libraryx\tPL:illumina" \
...
Next you need to index the bam file so that programs can randomly access the sorted data without reading the whole file. This command creates an index file with the same name as the input bam file, except with a .bai extension:
...
For help with interpreting the bam file, please have a look at the [Sequence Alignment/Map Format Specification](https://samtools.github.io/hts-specs/SAMv1.pdf).

### Check bam in IGV
To use IGV on Uppmax we recommend that you are connected via ThinLink. Please see instructions on how to use ThinLink in the course page in Canvas.
Alternatively you can install IGV on your local computer, download the files using scp, and look at them locally. This procedure is also described in the course page in Canvas.
The instructions below assume that you have logged in to Uppmax via ThinLink.
First use pwd to check if you are standing in the correct directory:
```bash
pwd
```
...
4. How can you estimate the coverage at a specific position in IGV?
...
The following command can be used to count the data lines (i.e. number of variants) in the vcf file:
```bash
...
We assume that you have logged in to Uppmax via ThinLink.\
...
Again we assume that you have logged in to Uppmax via ThinLink.\
If you have closed IGV please open it again as described above.\
...
do
  echo "Now analyzing: "$sample
  bwa mem -R "@RG\tID:${sample}\tPU:flowcellx_lanex\tSM:${sample}\tLB:libraryx\tPL:illumina" \
