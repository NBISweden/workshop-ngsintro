...
### Logon to a node {-}
This lab should be done on a compute node (not the login node). If you haven't already allocated a core today then please start with that. First check if you have any active allocations using this command:
```bash
squeue -u <username>
```
Where <username> should be replaced with your username. If you already have an active allocation please connect to that node as described below. If no jobs are listed please go ahead and allocate a core for this lab.
...
Make sure that you are located in this folder through out the exercise.
