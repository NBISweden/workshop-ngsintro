...
# Data description {-}
## Samples  {-}
The 1000 Genomes Project provides open access to the whole genome sequence of a large number of people. In this workshop we will use 3 samples from the low coverage phase of the 1000 Genomes project.
...
```
Although you don't have to specify wich versions of the toos to use, it is recommended to do so for reproducibility if you want to rerun the  exact same analyses later.  
...
In the call to `BWA` you need to add something called a *read group*, which contain information about how the reads were generated. This is required by HaplotypeCaller later on. We don't know exactly how the reads in the 1000 Genomes project were generated, but we will assume that each fastq file was generated from one library preparation (we call it *libraryx*), derived from one biological sample (*HG00097*), that was run on one lane of a flowcell (we call it *lanex_flowcellx*) on the Illumina machine, and we will add a read group called *readgroupx* containing this information. This is done with the code `-R "@RG\\tID:readgroupx\\tPU:lanex_flowcellx\\tSM:HG00097\\tLB:libraryx\\tPL:illumina"` in the command below. 
You don't need to understand the details about read groups here, but if ou would like to know more please read this article at [GATK-forum](https://gatk.broadinstitute.org/hc/en-us/articles/360035890671-Read-groups). The most important thing to know for today's workshop is that **readgroups are needed for HaplotypeCaller**, and **the SM flag should be the same as the sample name**. 
...
Please use this command to align the reads with ´BWA´, add the read group and sort the generated bam file:
```bash
...
```
Where  
-t 1 is the number of threads, which should be equal to the number of cores you booked. If you would have analysed the entire genome more threads would have been necessary.  You have to use a file redirect `>` for the output, otherwise it will be written to stdout (your screen).  
-R is the readgroup, and **when you run this command for another sample you have to change the SM tag to the new sample name**.  
