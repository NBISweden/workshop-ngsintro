...
### Logon to a node {-}
This lab should be done on a compute node (not the login node). First check if you already have an active allocation using this command:
```bash
squeue -u <username>
```
Where `<username>` should be replaced with your username.<br/>
If no jobs are listed you should allocate a job for this lab. If you already have an active allocation please connect to that node as described below.<br/>
