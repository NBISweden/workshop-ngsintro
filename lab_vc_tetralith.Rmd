...
```  
Note that your terminal prompt changed to something like `[nsc_username@tetralith1 ~]$` which means that you have entered your home directory on Tetralith. 
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('tetralith1 $ interactive -A ',nscid,' -t 04:00:00 -n 1'))
```
Note that your terminal prompt changed to something like `[<nsc_username>@n424 ~ ]$`, which means that you are now running on one of the compute nodes and you are still located in your home directory on Tetralith.
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('n424 $ singularity shell -B /proj/snic2020-13-57/users:/proj/g2020009/nobackup /proj/snic2020-13-57/ngsintro_homemade.5.sif'))
```
Note that your terminal prompt changed to `[hour:minute:second] <nsc_username>@fake-uppmax ~ $`. This means that you have moved into a "virtual computer" that mimics the UPPMAX environment, but you are still located in your home directory on Tetralith.   
To close the singularity container  later on type `exit`in the terminal, but don't do that now.
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('fake-uppmax $ mkdir /proj/',upid,'/nobackup/<nsc_username>/ngsworkflow\n'))
cat(paste0('fake-uppmax $ cd /proj/',upid,'/nobackup/<nsc_username>/ngsworkflow'))
```
...
**The rest of the instructions assumes that you runnig the UPPMAX singularity container from an interactive session on Tetralith, and that you are located in your *Cluster worksplace*, unless something else is specified.** 
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('fake-uppmax $ ln -s ',refdir,'/human_g1k_v37_chr2.fasta'))  
```
Do the same with the fastq files:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('fake-uppmax $ ln -s ',fastqdir,'/HG00097_1.fq\n')) 
cat(paste0('fake-uppmax $ ln -s ',fastqdir,'/HG00097_2.fq\n'))  
cat(paste0('fake-uppmax $ ln -s ',fastqdir,'/HG00100_1.fq\n'))
cat(paste0('fake-uppmax $ ln -s ',fastqdir,'/HG00100_2.fq\n'))
cat(paste0('fake-uppmax $ ln -s ',fastqdir,'/HG00101_1.fq\n')) 
cat(paste0('fake-uppmax $ ln -s ',fastqdir,'/HG00101_2.fq\n'))
```
...
```bash
fake-uppmax $ module load bioinfo-tools
```
This makes it possible to load the individual programs:
```bash
fake-uppmax $ module load FastQC/0.11.8
fake-uppmax $ module load bwa/0.7.17
fake-uppmax $ module load samtools/1.10
fake-uppmax $ module load GATK/4.1.4.1
```
...
```bash
fake-uppmax $ bwa index -a bwtsw human_g1k_v37_chr2.fasta
```
Check to see that several new files have been created using `ls -l`.
  
Generate a Samtool index: 
```bash
fake-uppmax $ samtools faidx human_g1k_v37_chr2.fasta
```
Check to see what file(s) were created using `ls -lrt`.

Genereate a GATK sequence dictionary:
```bash
fake-uppmax $ gatk --java-options -Xmx7g CreateSequenceDictionary -R human_g1k_v37_chr2.fasta -O human_g1k_v37_chr2.dict
```
...
```bash
fake-uppmax $ bwa mem -R "@RG\\tID:readgroupx\\tPU:lanex_flowcellx\\tSM:HG00097\\tLB:libraryx\\tPL:illumina" -t 1 human_g1k_v37_chr2.fasta HG00097_1.fq HG00097_2.fq | samtools sort > HG00097.bam
```
...
```bash
fake-uppmax $ samtools index HG00097.bam
```
...
```bash
fake-uppmax $ samtools view -H HG00097.bam 
```
...
```bash
fake-uppmax $ samtools view HG00097.bam | head -n 5 
```
...
```bash
fake-uppmax $ gatk --java-options -Xmx7g HaplotypeCaller -R human_g1k_v37_chr2.fasta -I HG00097.bam -O HG00097.vcf 
```
...
```bash
fake-uppmax $ grep '#CHROM' HG00097.vcf
```
...
```bash
fake-uppmax $ gatk --java-options -Xmx7g HaplotypeCaller -R human_g1k_v37_chr2.fasta -ERC GVCF -I <sample.bam -O <sample>.g.vcf 
```
...
```bash
fake-uppmax $ gatk --java-options -Xmx7g CombineGVCFs -R human_g1k_v37_chr2.fasta -V <sample1>.g.vcf -V <sample2>.g.vcf -V <sample3>.g.vcf -O cohort.g.vcf
```
Please replace `<sample1>`, `<sample2>`, `<sample3>` with the real sample names.
  
Then run GATK's `GenoteypeGVC` to generate a vcf file:
```bash
fake-uppmax $ gatk --java-options -Xmx7g GenotypeGVCFs -R human_g1k_v37_chr2.fasta -V cohort.g.vcf -O cohort.vcf
```
...
```bash
fake-uppmax $ gatk --java-options -Xmx7g MarkDuplicates \
      -I input.bam \
...
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('fake-uppmax $ gatk --java-options -Xmx7g MarkDuplicates -I HG00097.bam -O HG00097.md.bam -M HG00097_mdmetrics.txt'))
```
...
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('fake-uppmax $ gatk --java-options -Xmx7g BaseRecalibrator -R human_g1k_v37_chr2.fasta -I HG00097.md.bam> --known-sites ',refdir,'/1000G_phase1.snps.high_confidence.b37.chr2.vcf -O HG00097.recal.table\n'))
cat(paste0('fake-uppmax $ gatk --java-options -Xmx7g ApplyBQSR -R human_g1k_v37_chr2.fasta -I HG00097.md.bam --bqsr-recal-file HG00097.recal.table -O HG00097.recal.bam'))
```
...
Filter SNVs:
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('fake-uppmax $ gatk --java-options -Xmx7g SelectVariants \
  -R human_g1k_v37_chr2.fasta \
  -V cohort.vcf \
  --select-type-to-include SNP \
  -O cohort.snvs.vcf
  
fake-uppmax $ gatk --java-options -Xmx7g VariantFiltration \
  -R human_g1k_v37_chr2.fasta \
  -V cohort.snvs.vcf \
...
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('fake-uppmax $ gatk --java-options -Xmx7g MergeVcfs -I cohort.snvs.filtered.vcf -I cohort.indels.filtered.vcf -O cohort.filtered.vcf'))
```
