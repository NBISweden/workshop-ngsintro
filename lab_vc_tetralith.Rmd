...
### Interactive session 
When you connect to Tetralith you enter the *login node*, which is not intended for compute intensive tasks. You should therefore book a compute node, or in this case just one core of a node.
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('tetralith1 ~]$ interactive -A ',nscid,' -t 04:00:00 -n 1'))
```
Note that your terminal prompt changed to something like `[<nsc_username>@n424 ~ ]$`, which means that you are now running on one of the compute nodes. You are still located in your home directory on Tetralith.

### UPPMAX singularity container
We will use a singularity container (a virtual computer) that mimics the UPPMAX computing environment. Once you have started the singularity container your environment will look exactly as on UPPMAX, and the software used in this workshop will be avaiable through the module system.
...
```
Note that your terminal prompt changed to `[hour:minute:second] <nsc_username>@fake-uppmax ~ $`. This means that you have moved into a "virtual computer" that mimics the UPPMAX environment. You are still located in your home directory on Tetralith.   
