...
* You will use a singularity container (a virtual computer) that mimics the UPPMAX cluster, and once you have started the singularity container everything works like at UPPMAX.
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<nsc_username>@tetralith1$ interactive -A ',nscid,' -t 04:00:00 -n 1'))
```
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<nsc_username>@n424$ singularity shell -B /proj/snic2020-13-57/users:/proj/g2020009/nobackup /proj/snic2020-13-57/ngsintro_homemade.5.sif'))
```
Notise that your terminal prompt changed to `<nsc_username>`@fake-uppmax. This means that you have moved into a "virtual computer" that mimics the UPPMAX environment.   
To close the singularity container  later on, just type `exit`in the terminal, but don't do that now.

### Cluster workspace
Create a workspace folder for this exercise, and then move into it. This folder will be referred to as your *Cluster workspace* throughout this workshop.
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<nsc_username>@fake-uppmax$ mkdir /proj/',upid,'/nobackup/<nsc_username>/ngsworkflow\n'))
cat(paste0('<nsc_username>@fake-uppmax$ cd /proj/',upid,'/nobackup/<nsc_username>/ngsworkflow'))
```
When you exit the singularity container you will find the *Cluster workspace* and its content in:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<nsc_username>@tetralith1$ /proj/',nscid,'/users/<nsc_username>/ngsworkflow\n'))
```
When it is time to copy files to your *Local workspace* you must use this path, but worry, this is specified in the instructions then.
  
Unless something else is specified, the rest of the instructions assumes that you are runnig the UPPMAX singularity container from an interactive session on Tetralith, and that you are located in your *Clusetr worksplace*.  
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<nsc_username>@fake-uppmax$ ln -s ',refdir,'/human_g1k_v37_chr2.fasta'))  
```
Do the same with the fastq files:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<nsc_username>@fake-uppmax$ ln -s ',fastqdir,'/HG00097_1.fq\n')) 
cat(paste0('<nsc_username>@fake-uppmax$ ln -s ',fastqdir,'/HG00097_2.fq\n'))  
cat(paste0('<nsc_username>@fake-uppmax$ ln -s ',fastqdir,'/HG00100_1.fq\n'))
cat(paste0('<nsc_username>@fake-uppmax$ ln -s ',fastqdir,'/HG00100_2.fq\n'))
cat(paste0('<nsc_username>@fake-uppmax$ ln -s ',fastqdir,'/HG00101_1.fq\n')) 
cat(paste0('<nsc_username>@fake-uppmax$ ln -s ',fastqdir,'/HG00101_2.fq\n'))
```
...
```bash
<nsc_username>@fake-uppmax$ module load bioinfo-tools
```
This makes it possible to load the individual programs:
```bash
<nsc_username>@fake-uppmax$ module load FastQC/0.11.8
<nsc_username>@fake-uppmax$ module load bwa/0.7.17
<nsc_username>@fake-uppmax$ module load samtools/1.10
<nsc_username>@fake-uppmax$ module load GATK/4.1.4.1
```
...
```bash
<nsc_username>@fake-uppmax$ bwa index -a bwtsw human_g1k_v37_chr2.fasta
```
Check to see that several new files have been created using `ls -l`.
  
Generate a Samtool index: 
```bash
<nsc_username>@fake-uppmax$ samtools faidx human_g1k_v37_chr2.fasta
```
Check to see what file(s) were created using `ls -lrt`.

Genereate a GATK sequence dictionary:
```bash
<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g CreateSequenceDictionary -R human_g1k_v37_chr2.fasta -O human_g1k_v37_chr2.dict
```
...
```bash
<nsc_username>@fake-uppmax$ bwa mem -R "@RG\\tID:readgroupx\\tPU:lanex_flowcellx\\tSM:HG00097\\tLB:libraryx\\tPL:illumina" -t 1 human_g1k_v37_chr2.fasta HG00097_1.fq HG00097_2.fq | samtools sort > HG00097.bam
```
...
```bash
<nsc_username>@fake-uppmax$ samtools index HG00097.bam
```
...
```bash
<nsc_username>@fake-uppmax$ samtools view -H HG00097.bam 
```
...
```bash
<nsc_username>@fake-uppmax$ samtools view HG00097.bam | head -n 5 
```
...
Open a new terminal window on your laptop and navigate to your [local workspace](#preparelaptop), **but do not log in to NSC**. Copy the .bam and .bam.bai files you just generated with this command:
```{r,echo=FALSE,comment="",class.output="bash"}
...
```bash
<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g HaplotypeCaller -R human_g1k_v37_chr2.fasta -I HG00097.bam -O HG00097.vcf 
```
...
```bash
<nsc_username>@fake-uppmax$ grep '#CHROM' HG00097.vcf
```
...
Download the vcf file and its index to the *local workspace* on your laptop, just like you did with the bam file and its index earlier. Open a new terminal window on your laptop and navigate to your local workspace, **but do not log in to NSC**. Copy the files that you just generated:
...
```bash
<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g HaplotypeCaller -R human_g1k_v37_chr2.fasta -ERC GVCF -I <sample.bam -O <sample>.g.vcf 
```
...
```bash
<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g CombineGVCFs -R human_g1k_v37_chr2.fasta -V <sample1>.g.vcf -V <sample2>.g.vcf -V <sample3>.g.vcf -O cohort.g.vcf
```
Please replace `<sample1>`, `<sample2>`, `<sample3>` with the real sample names.
  
Then run GATK's `GenoteypeGVC` to generate a vcf file:
```bash
<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g GenotypeGVCFs -R human_g1k_v37_chr2.fasta -V cohort.g.vcf -O cohort.vcf
```
...
```bash
<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g MarkDuplicates \
      -I input.bam \
...
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g MarkDuplicates -I HG00097.bam -O HG00097.md.bam -M HG00097_mdmetrics.txt'))
```
...
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g BaseRecalibrator -R human_g1k_v37_chr2.fasta -I HG00097.md.bam> --known-sites ',refdir,'/1000G_phase1.snps.high_confidence.b37.chr2.vcf -O HG00097.recal.table\n'))
cat(paste0('<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g ApplyBQSR -R human_g1k_v37_chr2.fasta -I HG00097.md.bam --bqsr-recal-file HG00097.recal.table -O HG00097.recal.bam'))
```
...
Filter SNVs:
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g SelectVariants \
  -R human_g1k_v37_chr2.fasta \
  -V cohort.vcf \
  --select-type-to-include SNP \
  -O cohort.snvs.vcf
  
<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g VariantFiltration \
  -R human_g1k_v37_chr2.fasta \
  -V cohort.snvs.vcf \
...
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('<nsc_username>@fake-uppmax$ gatk --java-options -Xmx7g MergeVcfs -I cohort.snvs.filtered.vcf -I cohort.indels.filtered.vcf -O cohort.filtered.vcf'))
```
