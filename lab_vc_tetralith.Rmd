...
In the singularity container then type (this will make modules based on Java to load properly):
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('export MODULES_MACH=x86_64'))
```
...
**The rest of the instructions assumes that you are runnig the UPPMAX singularity container from an interactive session on Tetralith, and that you are located in your *cluster workspace*, unless something else is specified.** 
...
**Download the bam file to your laptop**  
...
**Optionally** - If you are interested in programming and have time, you can also try to write a simple bash script and run all steps automatically. Please check out the [bash script](#sbatchscript) section below if you would like to try this. 
...
./joint_genotyping.sh
```
If you would like more help with creating the bahs script, please look at our example solution:
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('#!/bin/bash
