...
```
Note that your terminal prompt changed to `<nsc_username>@fake-uppmax ~ $`. This means that you have moved into a "virtual computer" that mimics the UPPMAX environment. You are still located in your home directory on Tetralith.   
To close the singularity container  later on just type `exit`in the terminal, but don't do that now.

### Cluster workspace
While running the UPPMAX singularity cotainer, create a workspace folder for this exercise and move into it: 
```{r,echo=FALSE,comment="",class.output="bash"}
...
The *Cluster workspace* and its content can be reached also from outside of the UPPMAX singularity container in:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<nsc_username>@tetralith1$ /proj/',nscid,'/users/<nsc_username>/ngsworkflow\n'))
```
This is the path you must use when downloading files to your *Local workspace* later on (but don't worry, it will be specified in the instructions).
