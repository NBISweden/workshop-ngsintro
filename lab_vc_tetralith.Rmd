...
* In paths, please replace `<username>` with your NSC username.
* You will use a singularity image (a virtual computer) that mimics the UPPMAX cluster, and once you have started the singularity image everything works like at UPPMAX.
...
```bash
$ ssh -Y username@tetralith.nsc.liu.se
```  

### Interactive session 
When you login to Tetralith you enter the *login node*, which is not intended for compute intensive tasks. You should therefore book a compute node, or in this case just one core of a node.
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<username>@tetralith1$ interactive -A ',nscid,' -t 04:00:00 -n 1'))
```

### UPPMAX singularity container
We will use a singularity container (a virtual computer) that mimics the UPPMAX computing environment. Once you have started the singularity container your environment will look exaclty as on the Rackham cluster at UPPMAX, and all the software used in this workshop will be available through the module system just like on UPPMAX.  
Use this command to start the singularity container:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<username>@n424$ singularity shell -B /proj/snic2020-13-57/users:/proj/g2020009/nobackup /proj/snic2020-13-57/ngsintro_homemade.5.sif'))
```
Notise that your terminal prompt changed to `<username>`@fake-uppmax. This means that you have moved into a "virtual computer" that mimics the UPPMAX environment.   
To close the singularity container  later on, just type `exit`in the terminal, but don't do that now.

### Cluster workspace
Create a new folder that will be the workspace for this exercise, and then move into it. This folder will be referred to as your *Cluster workspace* throughout this workshop.
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<username>@fake-uppmax$ mkdir /proj/',upid,'/nobackup/<username>/ngsworkflow\n'))
cat(paste0('<username>@fake-uppmax$ cd /proj/',upid,'/nobackup/<username>/ngsworkflow'))
```
The newly created folder is reachable while you are running the singularity container that mimics the UPPMAX environment.
When you exit the singularity container, your newly created folder is available in:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<username>@tetralith1$ /proj/',nscid,'/users/<username>/ngsworkflow\n'))
```
When it is time to copy files to your *Local workspace* you will have to use the path to the folder that is reachable outside of the singularity container. Dont worry, this is specified in the instructions then.

### Symbolic links to data
From now on you will be working in The raw data files are located in the [Data](#Data) folder described above. Instead of copying the files to your workspace you should create symbolic links (soft-links) to them. Soft-linking files and folders allows you to work with them as if they were in your current directory, but without multiplying them.  
Create a symbolic link to the reference genome in your workspace:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<username>@fake-uppmax$ ln -s ',refdir,'/human_g1k_v37_chr2.fasta'))  
```
Do the same with the fastq files:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('<username>@fake-uppmax$ ln -s ',fastqdir,'/HG00097_1.fq\n')) 
cat(paste0('<username>@fake-uppmax$ ln -s ',fastqdir,'/HG00097_2.fq\n'))  
cat(paste0('<username>@fake-uppmax$ ln -s ',fastqdir,'/HG00100_1.fq\n'))
cat(paste0('<username>@fake-uppmax$ ln -s ',fastqdir,'/HG00100_2.fq\n'))
cat(paste0('<username>@fake-uppmax$ ln -s ',fastqdir,'/HG00101_1.fq\n')) 
cat(paste0('<username>@fake-uppmax$ ln -s ',fastqdir,'/HG00101_2.fq\n'))
```
...
```bash
<username>@fake-uppmax$ module load FastQC/0.11.8
<username>@fake-uppmax$ module load bwa/0.7.17
<username>@fake-uppmax$ module load samtools/1.10
<username>@fake-uppmax$ module load GATK/4.1.4.1
```
...
```bash
<username>@fake-uppmax$ bwa index -a bwtsw human_g1k_v37_chr2.fasta
```
Check to see that several new files have been created using `ls -l`.
  
Generate a Samtool index: 
```bash
<username>@fake-uppmax$ samtools faidx human_g1k_v37_chr2.fasta
```
Check to see what file(s) were created using `ls -lrt`.

Genereate a GATK sequence dictionary:
```bash
<username>@fake-uppmax$ gatk --java-options -Xmx7g CreateSequenceDictionary -R human_g1k_v37_chr2.fasta -O human_g1k_v37_chr2.dict
```
...
```bash
<username>@fake-uppmax$ bwa mem -R "@RG\\tID:readgroupx\\tPU:lanex_flowcellx\\tSM:HG00097\\tLB:libraryx\\tPL:illumina" -t 1 human_g1k_v37_chr2.fasta HG00097_1.fq HG00097_2.fq | samtools sort > HG00097.bam
```
...
```bash
<username>@fake-uppmax$ samtools index HG00097.bam
```
...
```bash
<username>@fake-uppmax$ samtools view -H HG00097.bam 
```
...
```bash
<username>@fake-uppmax$ samtools view HG00097.bam | head -n 5 
```
...
Navigate to your [local workspace](#preparelaptop), but do not log in to NSC. Copy the .bam and .bam.bai files you just generated with this command:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('scp <username>@tetralith.nsc.uu.se:/proj/',nscid,'/users/<username>/ngsworkflow/HG00097.bam* .'))
```
...
```bash
<username>@fake-uppmax$ gatk --java-options -Xmx7g HaplotypeCaller -R human_g1k_v37_chr2.fasta -I HG00097.bam -O HG00097.vcf 
```
...
```bash
<username>@fake-uppmax$ grep '#CHROM' HG00097.vcf
```
...
cat(paste0('scp <username>@tetralith.nsc.uu.se:/proj/',nscid,'/users/<username>/ngsworkflow/HG00097.vcf* .'))
```
...
## BWA mem 
Run `BWA mem` for all three samples in the data set. `BWA mem` should be run exactly [as above](#bwamem) but with the new sample names.
...
```bash
<username>@fake-uppmax$ gatk --java-options -Xmx7g HaplotypeCaller -R human_g1k_v37_chr2.fasta -ERC GVCF -I <sample.bam -O <sample>.g.vcf 
```
...
```bash
<username>@fake-uppmax$ gatk --java-options -Xmx7g CombineGVCFs -R human_g1k_v37_chr2.fasta -V <sample1>.g.vcf -V <sample2>.g.vcf -V <sample3>.g.vcf -O cohort.g.vcf
```
Please replace `<sample1>`, `<sample2>`, `<sample3>` with the real sample names.
  
Then run GATK's `GenoteypeGVC` to generate a vcf file:
```bash
<username>@fake-uppmax$ gatk --java-options -Xmx7g GenotypeGVCFs -R human_g1k_v37_chr2.fasta -V cohort.g.vcf -O cohort.vcf
```
...
```bash
<username>@fake-uppmax$ gatk --java-options -Xmx7g MarkDuplicates \
      -I input.bam \
...
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('<username>@fake-uppmax$ gatk --java-options -Xmx7g MarkDuplicates -I HG00097.bam -O HG00097.md.bam -M HG00097_mdmetrics.txt'))
```
...
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('<username>@fake-uppmax$ gatk --java-options -Xmx7g BaseRecalibrator -R human_g1k_v37_chr2.fasta -I HG00097.md.bam> --known-sites ',refdir,'/1000G_phase1.snps.high_confidence.b37.chr2.vcf -O HG00097.recal.table\n'))
cat(paste0('<username>@fake-uppmax$ gatk --java-options -Xmx7g ApplyBQSR -R human_g1k_v37_chr2.fasta -I HG00097.md.bam --bqsr-recal-file HG00097.recal.table -O HG00097.recal.bam'))
```
...
Filter SNVs:
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('<username>@fake-uppmax$ gatk --java-options -Xmx7g SelectVariants \
  -R human_g1k_v37_chr2.fasta \
  -V cohort.vcf \
  --select-type-to-include SNP \
  -O cohort.snvs.vcf
  
<username>@fake-uppmax$ gatk --java-options -Xmx7g VariantFiltration \
  -R human_g1k_v37_chr2.fasta \
  -V cohort.snvs.vcf \
...
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('<username>@fake-uppmax$ gatk --java-options -Xmx7g MergeVcfs -I cohort.snvs.filtered.vcf -I cohort.indels.filtered.vcf -O cohort.filtered.vcf'))
```
...
We recommend you to incorporate the new steps into an SBATCH script similar to the one you created above for [joint variant calling](#sbatchscript). You might find it helpful to try each step for one sample in the interactive terminal, so that you know that a particular command is working before you incorporate it in the script. Please try to complete the script for GATK's best practices workflow. If you run out of time you can sneak peek at our example solution below. 
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('#!/bin/bash
#SBATCH -A g2019031
#SBATCH -p core
