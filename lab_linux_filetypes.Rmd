...
Now you will need some files. To avoid all the course participants editing the same file all at once, undoing each other's edits, each participant will get their own copy of the needed files. The files are located in the folder `/sw/share/compstore/courses/ngsintro/filetypes`
...
cat(paste0("$ cp -r /sw/share/compstore/courses/ngsintro/filetypes /proj/",upid,"/nobackup/<username>/"))
```
...
`r fa1("exclamation-circle")` &nbsp; **IMPORTANT:** The export command affects only the terminal you type it in. If you have 2 terminals open, only the terminal you typed it in will have a modified path. If you close that terminal and open a new one, it will not have the modified path.
...
`r fa1("clipboard-list")` &nbsp; Run reference_indexer.
...
`r fa1("clipboard-list")` &nbsp; Align reads using **align_reads**, naming the output file **ad2.sam**, placed in the **1_alignment** folder.
...
`r fa1("clipboard-list")` &nbsp; To sort the BAM file we'll use the **sambam_tool** again, but specifying a different function, **-f sort** instead. Tell it to store the sorted BAM file in the **3_sorted** folder and name the file **ad2.sorted.bam**
...
`r fa1("clipboard-list")` &nbsp; Now when we have a sorted BAM file, we can index it. Use the command
...
`r fa1("lightbulb")` &nbsp; If you are using a Mac you might have to install the program [XQuartz](https://www.xquartz.org/), if you have not already installed that program. By using `-Y` in your ssh command you enable graphical transfer over ssh, but you will also have to have a program able to receive the graphics in order to display it.
...
<span class="ifa ifa-orange">`r fa2("lightbulb")`</span> **Optional**
...
`r fa1("clipboard-list")` &nbsp; Tell samtools that you want CRAM output (**`-C`**) and specify which reference genome it should use to do the CRAM conversion (**`-T`**)
...
If you have been fast to finish this lab and you still have time left (or just can't get enough of linux stuff), please have a look at the [loops lab](lab_loops.html) where you can learn the basics in bash programming using variables, loops and control statements.
