...
If you want to you can run all commands one by one in the terminal as you did in the first step of the workshop. However, you can also create  an SBACH scripts to run BWA mem and HaplotypeCaller automatically for all samples, see [below]{#runinscript} 
...
HaplotypeCaller should also be run for all three samples, but this time the output for each sample needs to be in g.vcf format. This is accompliched with a small change in the HaploteypCaller command (here examplified with the sample HG00097): 
```bash
gatk --java-options -Xmx7g HaplotypeCaller -R human_g1k_v37_chr2.fasta -ERC GVCF -I HG00097.bam -O HG00097.g.vcf 
```

## Joint genotyping
Once you have the g.vcf files for all samples you should run GATK's GenoteypeGVCF. This will generate a "normal" vcf file with variants for all samples (HG00097, HG00100 and HG00101).
```bash
gatk --java-options -Xmx7g GenotypeGVCFs -R human_g1k_v37_chr2.fasta --variant <sample1>.g.vcf --variant <sample2>.g.vcf --variant <sample3>.g.vcf -o <cohort>.vcf
```
Please replace <sample1>, <sample2>, <sample3> and <cohort> with the real sample names and name of the output file.  
...
#for s in "HG00097" "HG00100" "HG00101"
  #call bwa mem 
  #call to haplotypecaller -ERC
...
# Part three
Here we will expand the variant calling workflow to follow GATKs best practise short germline variant detection. 
