...
ln -s /sw/courses/ngsintro/reseq/data/fastq/HG00000_1.fq
ln -s /sw/courses/ngsintro/reseq/data/fastq/HG00000_2.fq
ln -s /sw/courses/ngsintro/reseq/data/fastq/HG00001_1.fq
ln -s /sw/courses/ngsintro/reseq/data/fastq/HG00001_2.fq
```
...
# Part two 
If there is still time you can continue to part two of the workshop. 
Here you will align reads and perform variant calling in two additional samples. This time you will emitt the output from variant calling in the individual samples in g.vcf format, and perform joint variant calling of the "cohort". 

## Align reads and emitt g.vcf files
Run BWA mem and HaplotypeCaller for all three samples in the data set. BWA mem shoulr be run exaclty [as above] {#bwamem} but you need to replace the sample name in the above command with the new sample names. , and [HaplotypeCaller] {#haplotypecaller} for all three samples in the data set.  Rerun the workflow for additional samples. If you want to repeat the procedure you can now use your saved commands and rerun the mapping and variant calling steps for at least one more sample (HG00100 or HG00101) from the course directory before continuing with the next step. Alternatively, you can copy the .g.vcf files of the two additional samples.
