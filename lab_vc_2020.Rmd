...
```
It is recommended that you specify the version number for reproducibility if you want to rerun the  exact same analyses later. Some programs, for example GATK, has different command syntax in different versions and therefore is important to know which version you used. The GATK commands in this tutorial is for GATK v 4.1.4.1 and forward, at least to 4.1.7.0 which is the latest in time of writing.
...
```
Again, check what new file was created using `ls -lrt`. 
...
We recommend you to incorporate the new steps into an SBATCH script similar to the one you created above for [joint variant calling](#sbatchscript). You might find it helpful to try each step for one sample in the interactive terminal, so that you know that a particular command is working before you incorporate it in the script. Please try to complete the script for GATK's best practice workflow. If you run out of time you can sneak peek at our example solution below. 
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('#!/bin/bash
#SBATCH -A g2019031
#SBATCH -p core
...
31. Did the extra refinement steps in GATK's best practice short variant discovery make any difference to these variants? 
  
**Note:** In this workshop we use public data from low coverage sequencing, that has been processed before publication. The difference between variants processed with our simplified workflow and the best practice workflow may be small in this case, but with a real-life data set it is recommended to follow the best practices workflow!
