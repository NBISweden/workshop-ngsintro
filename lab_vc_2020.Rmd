...
# Part three
In step 1-2 we have used a simplified workflow for variant calling because the priority in this course is to learn the concepts. Now we will look into the details, and make a workflow that follows GATKs best practise short germline variant detection. Please visit [GATKs web site](https://gatk.broadinstitute.org/hc/en-us/articles/360035535932-Germline-short-variant-discovery-SNPs-Indels-) to read more about their best practice. 
...
## Mark Duplicates
This tool locates and tags duplicate reads in a BAM or SAM file, where duplicate reads are defined as originating from a single fragment of DNA. Duplicates can arise during sample preparation e.g. library construction using PCR. Duplicate reads can also result from a single amplification cluster, incorrectly detected as multiple clusters by the optical sensor of the sequencing instrument. 
We use Picard to mark duplicate reads. This is the command to mark one single bam file:
```bash
