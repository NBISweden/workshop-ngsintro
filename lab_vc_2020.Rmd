...
`Integrative Genomics Viewer (IGV)` provides an interactive visualisation of the reads in a bam file. You can launch IGV from UPPMAX using your graphical forwarding, -X or -Y, as you may have already tried. However, here we will show how to run it on your laptop. If you have not used IGV on your laptop before, plase go to the IGV [download page](https://software.broadinstitute.org/software/igv/download), and follow the instructions. It will prompt you to fill in some information and agree to license. Launch the viewer through webstart. The 1.2 Gb version should be sufficient for our data.

You also need to download the bam file to your laptop. 
Navigate to your *local workspace*, but do not log in to UPPMAX. Copy the .bam file generated above with this command:
```{r,echo=FALSE,comment="",class.output="bash"}
