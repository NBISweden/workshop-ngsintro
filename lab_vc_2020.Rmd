...
## Variant Filtering
HaplotypeCaller is designed to be very sensitive, which good because it minimizes the chance of missing real variants. However, it means that the number of false positives can be quite large, so we need to filter the raw callset. GATK offers two ways to filter variants:  
...
**Note:** In this workshop we use public data from low coverage sequencing, that has been processed before publication. The difference between variants processed with our simplified workflow and the bet practise workflow may be small in this case, but with a real-life data set it is recommended to follow the best practices workflow!
