...
do
  ...
  #Fill in the code for running bwa-mem for each sample
  #Fill in the code for samtools index for each sample
  #Fill in the code for HaplotypeCaller for each sample
done
#Fill in the code for CombineGVCFs for all samples below
#Fill in the code for GenotypeGVCFs below
```
...
do
  echo "Now analyzing: "$sample
  bwa mem -R "@RG\\tID:readgroupx\\tPU:flowcellx_lanex\\tSM:"$sample"\\tLB:libraryx\\tPL:illumina" -t 1 human_g1k_v37_chr2.fasta $sample"_1.fq"\n $sample"_2.fq" | samtools sort > $sample".bam"
...
**Questions** 
* How many lines do the comined.g.vcf file have? (Hint: You don't have to open the file! You can use the linux command grep -v "#" filename.txt to extract all lines that don't start with "#", then "|", and then wc to count those lines.) 
