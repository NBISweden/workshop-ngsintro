...
Tools that compare (short) reads with a (large) reference genome needs genome index files to allow efficient access to the reference genome. Before we can begin analysing our samples we therefore need to create index files for each tool.  
...
Uppmax is a compute cluster with great analysis capacity,but when it comes to displaying data graphically it is oftem more efficient to dowload the data to your local computer. Now we will download the .html files generated by FastQC using SFTP, and look at them locally using a web-browser. First create a suitable workspace in your local computer and go into that. 
