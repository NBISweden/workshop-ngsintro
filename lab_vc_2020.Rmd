...
Now we will add some important refinement steps and make a workflow that follows [GATKs best practise for germline short variant discovery](https://gatk.broadinstitute.org/hc/en-us/articles/360035535932-Germline-short-variant-discovery-SNPs-Indels-). 
...
The new steps are listed below, and contain links to GATK's web where you can find detailed instructions.

![](data/ngs-workflow/3_best_practise.png){width=80%}

## Mark Duplicates
Sometimes a DNA fragment from the sample is sequenced multiple times, and therfore gives rise to multiple reads in the fastq file. This is called dupliate reads, and can occur due to PCR amplification during library preparation, or if one read cluster is incorrectly detected as multiple clusters by the sequencing instrument. 
If a duplicated read contains a variant allele, the ratio of the two alleles might be obscured which can lead to incorect genotyping. It is therefore recommended, in most cases, to mark duplicate reads so that they are counted as one read during genotyping.  
  
Please read about Mark Duplicates and why it is important [here](https://gatk.broadinstitute.org/hc/en-us/articles/360037225972-MarkDuplicates-Picard-). Try to follow the Usage Example in the bottom pf the page to mark duplicates in your data. 
...
Another source of genotyping errors is systematic biases in the assignment of base quality scores by the sequencinc instrument. The goal of the Base Quality Score Recalibration is therefore to correct for this.  
A general description of GATK's Base Quality Score Recalibration can be found [here](https://gatk.broadinstitute.org/hc/en-us/articles/360035890531-Base-Quality-Score-Recalibration-BQSR-). This is a two pass procedure, where you first use `BaseRecalibrator` to build a recalibration model, and then `ApplyBQSR` to recalibrate the basequalities in your bam file. 
`BaseRecalibrator` is documented [here](https://gatk.broadinstitute.org/hc/en-us/articles/360037593511-BaseRecalibrator) and `ApplyBQSR` is documented [here](https://gatk.broadinstitute.org/hc/en-us/articles/360037225212-ApplyBQSR).  

Try to follow the usage examples on those pages to recalibrate the base quality scores in your data.  
