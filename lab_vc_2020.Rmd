...
The last part of this workshop will take you through some important refinement steps that are needed to make a workflow that follows [GATKs best practise for germline short variant discovery](https://gatk.broadinstitute.org/hc/en-us/articles/360035535932-Germline-short-variant-discovery-SNPs-Indels-). The flowchart below shows how GATK's best practice short vartiant discovery should be applied to our three samples. You can either run the steps command by command in your reserved core, or include the steps in the SBATCH script that you created in Step 2 [above]{#sbatchscript}. The new steps are descibed below.

![](data/ngs-workflow/3_best_practise.png){width=80%}

## Mark Duplicates
Sometimes a DNA fragment is sequenced multiple times, which leads to multiple reads from the same fragment in the fastq file. This is called dupliate reads, and can occur due to PCR amplification during library preparation, or if one read cluster is incorrectly detected as multiple clusters by the sequencing instrument. 
...
In short, you first use `BaseRecalibrator` to build a recalibration model, and then `ApplyBQSR` to recalibrate the basequalities in your bam file using the model.
  
`BaseRecalibrator` is documented [here](https://gatk.broadinstitute.org/hc/en-us/articles/360037593511-BaseRecalibrator) and `ApplyBQSR` is documented [here](https://gatk.broadinstitute.org/hc/en-us/articles/360037225212-ApplyBQSR). Please follow the usage examples on those pages to recalibrate the base quality scores in your data.     
...
We recommend you to incorporate all analysis steps for thin a new version of the the SBATCH script that you created for the [joint variant calling]{#sbatchscript}. 

We advise you to try each step of the workflow in the interactive terminal for one sample, and paste successful commands in the SBATCH script.   
