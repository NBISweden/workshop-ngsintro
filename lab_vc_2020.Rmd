...
```
# Introduction
Whole genome sequencing (WGS) is a comprehensive method for analyzing entire genomes. This workshop will take you through the process of calling germline short variants (SNVs and INDELs) in whole geonome sequence data from three human samples. 
...
## Genomic region
The *LCT* gene on chromosome 2 encodes the lactase protein, which is responsible for the metabolism of lactose in mammals. Most mammals can not digest lactose as adults, but some humans tolerate lactose and can use it as energy source also in adulthood. Genetic variants upstream of the *LCT* gene control how lactose is tolerated in adults, and the variant rs4988235, located at at position chr2:136608646 in HG19, has been shown to lead to lactose persistence.  
  
To illustrate the process of variant detection, we will use sequencing data for the region chr2:136545000-136617000 in the 3 samples listed above. This region covers the *LCT* gene and the upstream *MCM6* gene, and contains the the rs4988235 variant.  
...
The majority of the analyses in this workshop will be done on Uppmax, but you will copy some of the resulting files to your laptop. Therefore, please start by creating a folder for this workshop on your local laptop. It is up to you where you want to put this, but it can for example be a folder called *ngsworkflow* on Desktop. You need to have write permission in this folder. The folder you create here will be referred to as *local workspace* throughout this workshop.  
...
Generate several BWA index files:
```bash
...
### FastQC  
You will first use `FastQC` to check the qualiy scores of the reads in the fastq files. The output is a .html document that shows the quality score along the reads, and other information.  
Start with creating a folder where the output from `FastQC` will be stored:
```bash
mkdir fastqc 
```
Then run `FastQC` on the first sample, and direct the output to your FastQC folder:
```bash
fastqc -q HG00097_1.fq HG00097_2.fq -o fastqc
```
  
### Dowload data to laptop {#downloaddata}
When it comes to viewing data graphically it is more efficient to dowload the data to your laptop and look at it therte. You should therefore download the .html files generated by `FastQC` using `scp`, and look at them using a web-browser on your laptop.  
...
Then open the two html files that you just dowloaded n a web browser.  
...
```
Where -t 1 is the number of threads to use (should be equal to the number of cores you booked). If you would have analysed the entire genome more cores and threads would have been necessary.  
...
Next you need to index the output bam file so that programs can randomly access the sorted data without reading the whole file. This creates an index file with the same name as the input bam file, except with a .bai extension.  
```bash
...
5. What is encoded in the @SQ tag?  
...
8. Can you estimate the coverage by looking at the bam file in IGV?   
...
### Question
9. What is the new file that ends with *idx*?
...
### Questions  
10. The vcf file is displayed on two rows; what is shown in the two rows in this case?  
11. What does it mean if a variant has two colors in the upper row?  
12. What colour heterozygout variants have in the second row?  
13. What does it mean if a variant is coloured in tourquise in the second row?  
...
## Questions 
13. How many lines do the comined.g.vcf file have? You can use the linux command `grep -v "#" filename.txt` to extract all lines in "filename.txt" that don't start with "#", then `|`, and then `wc` to count those lines.  
14. How many lines do the combined.vcf file have?  
15. Explain the difference in number of lines?  
16. In IGV, can you find the variant shown to be associated with lactose tolerance in adults?  
17. What is the approximate coverage in this region in all samples?  
18. Which samples do you think should avoid drinking milk? Any heterozygotes?  
19. Does this agree with the genotype in the vcf file?   
...
## Questions  
20. How many variants are present in the cohort.filtered.vcf file?  
21. How many variants have passed the filters?  
Look at the filtered variants in IGV and compare them to the vcf file generated in step two [above](#jointvc). Zoom in to the variants in *LCT* and *MCM6*.  
22. Did the extra refinement steps in GATK's best practise short variant discovery make any difference to these variants? 
