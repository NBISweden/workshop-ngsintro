...
1. The first part of the workshop will guide you through a basic variant calling workflow in one sample. The aim is to get familiar with the bam and vcf file formats, and how to interpret the results of variant calling in Integrative Genomics Viewer (IGV).
2. If you have time, the next part of the workflow will show you how to perform joint variant calling on three samples. The goal of this part of the workshop is to understant the differences betwen g.vcf files and vcf files, and You will combine individual linux commands into a workflow that can be started as an SBATCH script.  
3. The last part of the exercise, if you have time, will take you through the GATK best practise short variant detection. 
