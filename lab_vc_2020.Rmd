...
If you need some more help you can look at our example solution for HG00097 below. 
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
...
The goal of the Base Quality Score Recalibration is to correct for systematic bias that affect the assignment of base quality scores by the sequencer. A genreal description of the procedure can be found [here](https://gatk.broadinstitute.org/hc/en-us/articles/360035890531-Base-Quality-Score-Recalibration-BQSR-)
This is a two pass procedure, where you should first use GATK's BaseRecalibrator to build a recalibration model, and then use GATK's ApplyBQSR to recalibrate the basequalities in your bam file using the model you created in the first step. 
BaseRecalibrator is documented [here](https://gatk.broadinstitute.org/hc/en-us/articles/360037593511-BaseRecalibrator) and ApplyBQSR is documented [here](https://gatk.broadinstitute.org/hc/en-us/articles/360037225212-ApplyBQSR). Try to follow the usage examples on those pages to recalibrate the base quality scores in your data.  
  
Note: BaseRecalibrator requres a file with known SNPs as input. This file is available in the data folder on Uppmax in  
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0(refdir,"/1000G_phase1.snps.high_confidence.b37.chr2.vcf"))
```

If you would like more help you can sneek peek at our example solution for HG00097 below.  
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
...
GATK's best practise short variant discovery is complex, where several steps should be performed on individual sample level followed by a workflow for the joint variant calling. When you have three samples it is possible to run the analysis manually in the command line, but runing the analysis in an automated script is much more efficient. 
We recommend you to incorporate all analysis steps in a new version of the the SBATCH script that you created for the [joint variant calling]{#sbatchscript}. 
...
Please try yourselfe to complete the script for GATK's best practise workflow. If you run out of time you can sneek peek at our example solution:
```{r,accordion=TRUE,block.title=TRUE,echo=FALSE,comment="",class.output="sh"}
cat(paste0('#!/bin/bash
#SBATCH -A g2019031
#SBATCH -p core
