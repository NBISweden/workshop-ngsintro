...
2. If you have time, the next part of the workflow will show you how to perform joint variant calling on three samples. The aim is to understant the differences betwen g.vcf files and vcf files, and to learn how to combine individual linux commands into a workflow that can be started as an SBATCH script.  
...
Now lets start the first part of the workshop, which will guide you through a basic variant calling workflow in one sample.
## Quality control
...
Now it is time to align reads and perform variant calling in all three samples. This time variant calls in each sample will first be emitted g.vcf format. The individual g.vcf fiels will subsequenctly be combined with GATK's `CombineGVCFs`, and translated into vcf format with GATK' `GenotypeGVCFs`. 
![](data/ngs-workflow/2_jointvc.png)


You can run all commands one by one in the terminal as you did in the first step of the workshop. However, since the same steps needs to be repeated for all samples, this is a good opportunity to learn how to write a simple SBATCH script to run all steps automatically for all samples. If you are interested in this, please see the [SBATCH script]{#runinscript} section below. 
