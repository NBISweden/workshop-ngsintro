...
The raw fastq files are located in the [Data](#Data) folder described above. In stead of copying the files to your workspace you should create symbolic links (soft-links) to them. We do this because fastq are very large, and copying them would use up a lot of storage space. Soft-linking files and folders allows us to work with those files as if they were actually there. Use pwd to check if you are standing in the correct directory. You should be standing here:
...
Index files allow efficient random access to large files. The tools BWA, Samtools and Picard need different indices of the reference fasta file, so they must be generated before we can proceed to analysing our samples.  
...
```
Check to see that several new files have been created using `ls -lrt`.
