...
```
Please adjust the requested time depending on what lab you will be working on.
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('offline-uppmax$ source /uppmax_init'))
```
...
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('offline-uppmax$ mkdir /proj/',upid,'/nobackup/<nsc_username>/linux_tutorial\n'))
cat(paste0('offline-uppmax$ cd /proj/',upid,'/nobackup/<nsc_username>/linux_tutorial'))
```
All files and folders that you create in `r paste0('/proj/',upid,'/nobackup/nsc_username/')` while running the UPPMAX singularity container can be reached also from outside of the container, in this folder on Tetralith:
```{r,echo=FALSE,comment="",class.output="bash"}
cat(paste0('tetralith$ /proj/',nscid,'/users/<nsc_username>/\n'))
```

When you have the UPPMAX singularity image up and running you can follow regular lab instructions, except that you should not connect to UPPMAX or book a node, and you should work in the cluster workspace defined here.
